...
SESSION_EXIT=0
zsh -ic "claude --model 'sonnet' --effort 'high' --dangerously-skip-permissions '/agmsg actas setup-model-effort-claude then wait idle. Execution instructions will arrive as a prompt typed into this pane; an identical copy is also pushed to your agmsg inbox (treat both as ONE task — ignore the duplicate). Do not start any work until the instructions arrive.'"
SESSION_EXIT=$?
