...
- model を `config-lib.sh` の関数で検証する。手編集が公式にサポートされた経路である
  ...
  検証する**。ただし扱いは effort と同じ**レイヤー単位の無効化**である: 警告して
  そのレイヤーの model だけを捨て、次のレイヤー、最終的に既定値へ落とす。**resolver 全体を
  exit 2 にはしない**（config は人が手編集する場所であり、1 つの打ち間違いでディスパッチ全体を
  止めるのは過剰）。不正値が出力へ残らないことが要件である。

  これは §4 の `roles.json` / `prewarm.json` 再検証が**die する**のと意図的に非対称である。
  config レイヤーは人間の入力なので劣化させて続行し、機械が生成した中間ファイルは 1 バイトでも
  想定外なら止める。fail-closed が要るのはコマンド合成の直前だけであり、そこは §4 が担う。
