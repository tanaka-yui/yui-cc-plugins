...
### V1 の格下げ（2026-09-01、実装フェーズの反例により）

上の実測は有効だが、**無条件には成立しない。** 実装フェーズで反例を観測した。

> **TUI へタイプ入力して始まったターンでは、block が再駆動することを確認した。
> agmsg 注入で始まったターンについては未検証であり、反例を観測した。**

反例の観測条件（再現・反証できるよう具体的に残す）:

| 項目 | 値 |
|---|---|
| 日付 | 2026-09-01 |
| モデル / effort | gpt-5.6-terra / high |
| 実行モード | 対話 TUI |
| context | 68% 使用 |
| ターンの開始 | **parent からの agmsg メッセージ**（codex bridge / app-server 経由） |
| 症状 | final 応答の直後に idle。gate を手動実行すると `decision=block` が返る |

上の V1 実測との決定的な差は **ターンの開始方法**である。私の実測は TUI へのタイプ入力で
始まったターンだった。

**別の仮説も残す。** 反例を観測した時点で exec は `completion-gate.sh` 自身を編集していた。
編集途中に構文エラーや非ゼロ終了の瞬間があれば hook は block を返せない。これが真なら
V1 は無傷である。切り分けは未了。

**どちらであっても設計は変わらない。** block はベストエフォートの第一層であり、
実効的な期限執行は `recovery-tick.sh`（§4 C1）が担う。V1 が偽でも C1 は成立する。
