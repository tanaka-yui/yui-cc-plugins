...
#### 必須ロールの集合は段階で変わる（plan round 2 finding 2）
...
| 段階 | 必須 | 許される追加 |
|---|---|---|
| **Stage 1**（1 ロール） | `design` のみ | 無し。**`exec` を持たないことが妥当である** |
| `review_mode=off`（F-a 以降） | `design` + `exec` | 無し |
| `review_mode=on`（F-b 以降） | `design` + `exec` | `design_review` / `exec_review` |

`validate_workers_snapshot` は**期待するロール集合を第 2 引数で受け取る**。
呼び出し側（launch / sweep / cleanup / gate）が同じ集合を渡すので、段階が進んでも
validator を書き換えずに済む。

```bash
validate_workers_snapshot "$doc" "${DISPATCH_REQUIRED_ROLES:-design}"
```

**ダミーの `exec` を fixture に足して検査を通してはならない。**テストの fixture は
実際の生成物と同じ形でなければ、乖離を検出できない。
...
### 主張には referent が要る（機械的な形。親の指示 2026-09-04）

**4 回起きたということは、注意力の問題ではない。**「振る舞いを述べる散文は書くのが安く
満足感があり、それを実装する手順は高価で後回しにしやすく、両者を出会わせる強制力が
書く工程に何も無い」という、この種の文書の性質である。注意深さは 4 回失敗したので、
注意深さは対策にならない。**規律を、実際に走らせられる検査に変える。**

> **`SKILL.md` / `references/` / **計画** のどこであれ、「このソフトウェアは X をする」と
> 主張する文には、**X を真にするタスクまたは step の名前**が伴わなければならない。
> referent を持たない主張が 1 つでもある計画は完成していない。**

これはレビュー依頼を「書いてから送る」2 手順から `review-request.sh` の 1 コールへ
変えたのと同じ動きである — 飛ばせる手順を、飛ばせない構造にする。

**適用例**（plan round 2 で実際に見つかったもの）:

| 主張 | 要求される referent | round 2 時点 |
|---|---|---|
| SKILL.md「ローカル merge する」 | merge を実行するタスク | **無かった** → 成果が親へ届かない |
| SKILL.md「cleanup が worktree を除去する」 | 除去するタスク。**かつ merge より後**であること | 除去はあったが**順序が逆**で成果が消える |
| manifest「並列」「4 ロール」 | 並列と 4 ロールを実装するタスク | **無い**（Stage 1 は 1 ロール） |

**順序も referent の一部である。**「cleanup が除去する」に対応するタスクがあっても、
それが merge より前に走るなら主張は満たされていない。
...
4. **正本と実装の乖離を機械的に検出できる箇所はテストにする。**
   `test-skill-script-refs.sh` はスクリプトの実在に加え、**上の referent 規則**も検査する:
   SKILL.md と manifest の主張語彙が、実在するタスク／step に対応していること
