...
2. DOC を 1 回だけ読む（下記スニペット）。parse 失敗・読み取り不能は exit 1
3. --name が与えられたとき: (.runners | type) == "array" を検証。違反は exit 2
4. --name が与えられたとき: 一致件数がちょうど 1 であることを検証。違反は exit 2
5. --set <*_effort> があるときだけ、レコードの engine を読み allowlist と照合。
   違反は exit 2。書き込みモード専用
...
**すべての外部コマンド呼び出しに明示分岐を書く（スニペットで固定する）。**
...
散文で「失敗は exit 1」と書くだけでは裸の形が書かれるため、4 箇所ともスニペットを焼く。
...
手順 8 の jq 失敗ハンドラは、`--arg` 契約と手順 3 の型検証が効いていればほぼ到達しない
**意図的に到達困難な安全弁**である（`{"runners":[1,2]}` のように要素がオブジェクトでない
レジストリでだけ踏む）。RE9 がこれを守っているわけではない点を明記しておく。

**jq への値の渡し方（セキュリティ上 load-bearing）**:

**この契約は手順 5 / 7a / 8 のすべての jq 呼び出しに適用する。**
...
- **登録が 5 件を超えるとき**は、転置形を出すのを次の**優先集合**に絞る:
  `default` と、**解決値**としての `design_runner` / `review_runner` / `exec_choice`
  が参照している runner。優先集合が空（役割キーが全部 `"ask"` / 未設定 / 不正名）なら
  **登録順の先頭 5 件**を使う。残りは 1 行要約にする。
  ...
  - 閾値 5 は AskUserQuestion の 4 択上限とは無関係の**表示上の**閾値で、
    6 runner = 36 行が実用上の上限だからそこで切る。M1 の「先頭 4 件」（選択肢の
    上限）とは別の数え方であることを doc にも書く。
...
3. **最終枠。規則 2 より優先して確保する**（先に規則 2 で 4 枠を埋めてしまうと、
   ...
   | 条件 | EN | JA |
   |---|---|---|
   | 設定済み かつ 実効既定値と異なる | `back to the default (<d>)` | `既定に戻す（<d>）` |
...
**選択肢は必ず 2 つ以上になる。** 規則 2 の除外は高々 2 件だが現在値と実効既定値は
一致しうるので実質高々 2 件、**最小のプールである claude `*_model`（3 件）でも
最低 1 件は残る**。設定済みなら規則 3 がさらに 1 枠を足す。codex model は候補導出
step 3 / 4 が下限を保証する。実装者が床を再導出しなくて済むよう、この 1 文を doc にも書く。
...
4. それでも枠が余ったら**枠を減らす**。ダミーは置かない。設定済みなら規則 3 の最終枠が
   床を支え、未設定なら step 1 が何も除去しないのでプールに 1 件以上残る。
...
| 〃 | `### S3-M`（新設） | S3-M 本体。下位見出しは `####`、英日で個数・順序も一致 |
| 〃 | S3-M 内の候補プール表 | claude / codex の `*_effort` **2 行を逐語**で持つ（SU12 のアンカー）。ヘッダと `codex *_model` セルは英語 |
| 〃 | S3-M 内の `*_model` 拒否条件 | 空白のみ / `'` `"` `` ` `` `$` `\` / `[[:cntrl:]]` を拒否する旨。**「S3 の選択肢 2 経由のみ。選択肢 1 / 3 の First-run setup は検証しない」という限定句を必ず添える**（§1.3.1） |
...
| 〃 | 保守手順 28 本文 | `runners-edit.sh` の契約、SU10-SU16 の needle 一覧、`setup-mode.md` が SU12 のアンカー 2 行（§2.4 と 1 バイト一致）と S7 温存 3 文を維持する義務 |
...
- **主語が「役割キー 5 つ」に限定されていて変更後も真な同一命題 4 箇所**:
  `SKILL.md:490` / `guide-ja.md:1494` / `guide-ja.md:1494-1497` の「3 通りの設定経路」と
  **`CLAUDE.md:193`（保守手順 19）「書き込みは全経路とも `config-edit.sh` を通すこと」**。
  3 つだけ挙げて 1 つ落とすと誤って追記されるので 4 つとも列挙する。
...
| RE8 | 壊れた JSON では exit 1 かつ元ファイルを破壊しない。**読み取り権限の無い既存ファイルも exit 1**（`[[ $EUID -ne 0 ]]` ガード付き、`chmod` を戻してから assert）。どちらも jq 由来の rc=5 / rc=2 が漏れていないこと |
| RE9 | temp の残骸がゼロ。経路は 6 つ: 成功 / `*_model` 検証エラー exit 2 / effort 検証エラー exit 2 / 未知 `--name` exit 2 / `--dry-run` / `mv` 失敗。**ただし残骸ゼロを保証するのは trap であって順序ではない**（順序は RE9d が見る） |
...
| RE9d | **順序の直接検証。** `chmod 555` の親に置いた既存 `runners.json` に対し RE9 の経路 2/3/4（`*_model` 検証 / effort 検証 / 未知 `--name`）を実行し、**exit 2** を assert する。誤った順序（mktemp が検証より前）の実装は mktemp が Permission denied で落ちて rc=1 になる。**残骸ゼロではなく exit code が順序の観測窓である。** RE9c と同じ root ガードと `chmod` 復元を行う |
| RE9e | **`--dry-run` の no-mktemp 規則。** `chmod 555` の親で `--dry-run` を実行し、**exit 0 かつ正しいレコードを stdout へ出す**ことを assert する。mktemp する実装はここで落ちる。RE9c と同じ root ガードと `chmod` 復元を行う |
| RE10 | `--get` / `--show` は非破壊。フィールド未設定は空文字で exit 0。**ファイル不在時も `--get` は空 + exit 0、`--show` は `{}` + exit 0**。**`--show --name <未登録名>` と `--get --name <未登録名>` はどちらも exit 2**（フィールド未設定の exit 0 と区別する） |
...
| RE15 | **書き込みモード、または `--name` 付き読み取りで** `.runners` 欠落 / `null` / 非配列（オブジェクト）に対して **exit 2**（exit 1 では不可）かつファイル無変更。特にオブジェクトが配列へ化けないこと。`--name` 無しの `--show` は素通しで exit 0 |
| RE16 | `--name` が 2 件一致する（重複 name）レジストリでは exit 2 かつファイル無変更。**隣接ケース**: `a"b$c` のような `"` / `$` を含む runner 名でも `--set` が成功し当該レコードだけが更新される（`--name` を jq 式へ補間する実装を弾く。RE14b の `--name` 版） |
...
| RE18 | `--dry-run` は当該レコードだけを stdout へ出し、**ファイルを変更しない**。`--get` / `--show` との併用は exit 2。**ファイル不在での `--dry-run` は exit 2**。**dry-run の jq が失敗しても rc が 0/1/2 の範囲に収まる** |
...
| id | 対象 | needle（逐語） |
|---|---|---|
| SU10 | `setup-mode.md` | `S3-M` / 6 フィールド名 / `edit an existing runner's models and efforts`（S3 選択肢 2）/ `keep (` / `back to the default (` / `unset it (not review-capable)` / `unset (not review-capable)` / `no longer be chosen as the reviewer` / `gpt-5.6-sol` / `the role keys only` / `mkdir -p .dispatch` / `shadows the global layer` / `[[:cntrl:]]` / `only through option 2`（拒否条件の限定句） |
...
| SU15 | `setup-mode-ja.md` | 6 フィールド名 / `S3-M` / `登録済み runner の model / effort を編集` / `変更なし（現在:` / `既定に戻す（` / `未設定（レビュアーに選べません）` / `未設定に戻す（レビュアーに選べません）` / `レビュアーに選べなくなります` / `gpt-5.6-sol` / `役割キーのみ` / `mkdir -p .dispatch` / `グローバルより優先されることをユーザーに伝える` / `選択肢 2 経由のみ`。**加えて `####` の個数が `setup-mode.md` と一致し、かつ両方とも 6 個以上**（パリティだけだと両方 0 個で緑になり、S3-M の小節構造が丸ごと欠けても検出できない） |
| SU16 | `setup-mode.md` / `setup-mode-ja.md` | §2.6 温存表の逐語 3 文（英日それぞれ）。**加えて `runners-edit.sh` の記述が `config-edit.sh` の記述より前の行に現れる**（`grep -n` の行番号比較 1 行で S7 の順序を固定する） |
...
- **順方向**: 引数なし usage に現れる long flag 7 個がすべて `setup-mode.md` に現れる。
  doc の書き漏らしを検出する。
- **逆方向**: `setup-mode.md` の **`runners-edit.sh` を含む fenced code block
  （```` ```bash ```` で始まるもの）だけ**を対象に `--[a-z][a-z-]*` を抽出し、
  ...
  - 範囲を code block に限定すれば `config-edit.sh` のフラグを誤検出しない。
...
その場合に触る箇所は **12 個**ある。「§1.3 の条件と RE4 を落とすだけ」では済まない。
...
| 7 | §2.4 M1 の `'` 除外 | **変更不要**（§1.3 に依存していない）。ただし L の「§1.3 の deny-list の対象外」という参照句を「§2.6 の S7 クォート要件」へ差し替える |
| 8 | §2.6 のクォート規約の相互参照 | 参照先を §1.3.1 から #4 の残置要件へ差し替える |
| 9 | §3 同期表の「S3-M（`*_model` の拒否条件）」行 | `'` 限定へ縮小 |
| 10 | SU10 の `[[:cntrl:]]` / `only through option 2` needle、SU15 の `選択肢 2 経由のみ` | 削除または `'` 限定へ |
| 11 | **RE4** と **RE14a** | RE4 の拒否アサーションを `'` のみへ縮小。**RE14a は期待値が反転する** — `exit 2` + `cmp` バイト同一ではなく、`exit 0` + `.plan_model` がペイロードとリテラル一致 + `.command` / `.engine` / `.name` 不変 + 他 runner と `default` 不変 に書き換える |
| 12 | 「明示的に採らなかった対策」の該当 2 行と本節 | 削除 |

**RE14b だけが両分岐で成立する**（`opus[1m]` は拒否文字を含まないので、どちらでも
exit 0 + 往復になる）。RE14a は分岐依存なので上表 #11 のとおり書き換えが要る。
...
| 値やフィールド名の補間による `.command` 書き換え / `$ENV` 読み出し | §1.4 の `--arg` 契約（手順 5 / 7a / 8 すべてに適用）+ RE14a / RE14b / RE16 の隣接ケース + RE2 の 3 モード |
...
| codex `review_model` の unset で次回ディスパッチが `die` | §2.4 の 3 段防御（S1 表示 / 選択肢 description / S6 警告文言）+ SU10 / SU15 の needle 3 種 |
