...
レビューで挙がったが、この作業では意図的に手を付けなかったもの。

**動作に影響しうるもの**

- `send.sh` に timeout が無い。inbox 記録をタイプ入力の後ろへ移したので**唯一の wake 手段を塞ぐことは無くなった**が、
  ハングすれば呼び出し元は依然としてそこで止まる。macOS には `timeout` / `gtimeout` が無いため、
  素直な解決策が無い（`docs/notification-gaps.md` 参照）
- 「画面上で最後の `❯` 行が入力欄」は観測に基づく前提で、テストで固定した不変条件ではない。
  破れた場合の失敗方向は **false negative**（詰まりの見逃し）で、元のバグの再発に相当する。
  codex ペインのように `❯` 行を持たない宛先では検証が no-op になる（fail-open）。文書はこの限定を明記済み
- 修正前は `--retries 0` を渡すと、配送に成功していても `exit 1` を返した。現状これを渡す呼び出し元は無い。2026-08-13 に `verify_input_box()` を修正し、検証 1 回・再送 0 回の契約へ是正済み。SP19-SP24 で境界値、fail-open、最後の再送後の再検証を回帰検証する
- `notify_parent_once` は通知成功時にしか marker を書かないため、検証が false negative を出し続けると
  親への通知が 15 秒ごとに繰り返される。上限は設けていない
...
- `send-prompt-exempt:` マーカーによる除外は、スクリプトで 3 行・文書で 1 行の窓を持つ。
  意図的な escape hatch だが、新しい正当な `cmux send` を足すときはマーカーが必要になる
- `verify_input_box` が 1 を返す経路でも agmsg 記録が行われる性質は、コード読解でのみ確認しており
  テストで固定していない
- `SP18`（記録がタイプ入力の後であること）だけ stderr のリダイレクトが無い

## 実ディスパッチ E2E（2026-08-13）

配送修正後の `cmux-team-dispatch-task` v1.16.0 で、実タスク 2 件を並列ディスパッチした。
題材は上の「残課題」から 2 項目（`--retries 0` の終了コード / `CS1` の検査範囲）。
設定は design=claude / exec=codex 固定、レビューモード有効、prewarm 有効。

### 配送は全経路で成立した

両タスクとも Phase A-R（codex がプランをレビュー）→ codex 実装 → Phase B-R
（設計 opus ペインがコードレビュー）→ 完了通知、まで完走した。

タスク 1 だけで 7 通が outbox 経由になった。最大は Phase B 実行指示の **7,944 バイト**で、
本件の発端となった詰まったメッセージ（約 1.3KB）の 6 倍にあたる。**入力欄に実際に
タイプされたのは各 1 行のポインタだけ**で、詰まりは 1 度も起きなかった。

### 見つかった問題

**プラグイン更新後はスキルの再読み込みが必要。** `install.sh` を実行しても、実行中の
セッションは読み込み済みの旧 SKILL.md を保持し続ける。旧本文には削除済みの
`--message-type agmsg` を渡す手順が含まれるため、そのまま実行すると
`was removed` の die に当たってディスパッチが失敗する。`/reload-plugins` が必要。

**子セッションが親のチェックアウトを編集しうる。** タスク 1 の子は、担当ファイルを
ワークツリーではなく親リポジトリのチェックアウト側でも編集していた。原因は、
プロンプトが `SKILL_DIR` / `STATUS_DIR` を**親チェックアウトの絶対パス**で渡している
一方、タスク自体がその絶対パス配下のファイル（`scripts/send-prompt.sh`）を対象に
していたため、同名のコピーが 2 か所に見えていたこと。

今回は影響が限定的だった（ワークツリー側がレビューを通った正本で、親側は破棄した
中間ドラフトだった）が、気づかなければ古い版を取り込む事故になりうる。プラグイン
