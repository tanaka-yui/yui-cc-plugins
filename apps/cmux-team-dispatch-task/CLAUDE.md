...
| `skills/cmux-team-dispatch-task/scripts/ensure-agmsg-ready.sh` | agmsg watcher の setup guard。`Monitor` tool を持たないハーネスでも watcher の有無を自ら判定し、無ければ起動する（`AGMSG-DIRECTIVE:` に人手で従う旧経路の代替） |
| `skills/cmux-team-dispatch-task/scripts/agmsg-path.sh` | agmsg の ready sentinel パス（`ready.<team>__<agent>`）を組み立てる source 専用ヘルパー。team 名の percent-encoding を guard と `send-prompt.sh` で共有する |
...
| 永続プロセス | agmsg インストール時、guard がペインごとに inbox watcher を nohup で起動する。composite id の watcher は、そのペイン自身の liveness token が stale になってから `AGMSG_WATCH_INTERVAL` 秒以内に自己終了する。SIGKILL で pane が終了すると EXIT trap が走らず pidfile / sentinel が残るため、その場合の掃除は手動になる | なし |
...
12. **`message_type` 廃止後の判定**が SKILL.md / guide-ja.md / README.md / CLAUDE.md で一致しているか確認。通知トランスポートの質問も config キーも存在せず、`AGMSG_INSTALLED` は `~/.agents/skills/agmsg/scripts/send.sh` の存在を初期値とし、Step 1g の guard 呼び出し（`ensure-agmsg-ready.sh`）が配線に失敗するとバイナリ自体は存在していても `false` へ降格すること。`launch-workspace.sh` / `prewarm-panes.sh` が `--message-type` を `was removed` を含む die で拒否し、agmsg 配線は `--agmsg-team` の有無で決まること。`AGMSG_INSTALLED=true` のときは monitor-dispatch.sh を起動しないこと、status.json / signal は不変であることを検証。子プロンプトの status protocol に「status.json 書き込み直後の必須完了通知（`send-prompt.sh --label dispatch-notify` の 1 回呼び出し）」が含まれること、Step 1g が `AGMSG-DIRECTIVE:` 行を人手で追わせず `ensure-agmsg-ready.sh` を直接呼び出して自セッションの watcher を起動する guard 呼び出しになっていることを検証。**guard の検証項目**: 引数検証・候補判定・stale 掃除・起動・待機・composite 判定・ログ削除の一連が仕様どおりであること（watcher は記録・返信用で wake 手段ではない旨の注記も含む）。回帰は `bash test/test-message-type-removed.sh`（MT1-MT3）に加え、guard 本体は `bash test/test-agmsg-ready.sh`（AR1-AR26。AR15b / AR18 / AR3f は欠番）、Step 1g のブロックは `bash test/test-agmsg-skill-block.sh`（AG1-4）で検証する。`run_guard` は**上限付き**で guard を走らせる（バックグラウンドのサブシェル + `kill -0` ポーリング。`timeout` / `gtimeout` は macOS に無いので使わない）— 上限が無いと fd 漏れの退行でスイートが `--- failures ---` すら出さずに無限ハングし、CI が終わらない。AR8 は意図的にスイートの先頭近く（AR16 群の直後）に置く — 上限に掛かった診断は「どの呼び出しが返らなかったか」しか言わないので、fd 漏れそのものを名指しする AR8 が先に落ちる方が原因を特定しやすいため
13. prewarm.json が `design` / 任意の `review` / `executors`（キーは `claude` / `codex` の最大2つ、in-session 成立時は `{}`）の role-aware schema で、固定 `exec_choice` が未選択 pane を抑止すること。各ロールのオブジェクトが `watcher` キーを持ち、そのペインの初期プロンプトに guard 呼び出しが注入されていれば `"guard-injected"`、配線失敗、または `SCRIPT_DIR` の空白・シェルメタ文字で注入できなければ `"none"` であること。all-Codex 固定構成は3ペインで claude executor wiring が0件。配送順序と未assigned時のstatus非汚染は維持する。
...
15. **配送の `send-prompt.sh` 一本化**が SKILL.md / guide-ja.md / README.md / CLAUDE.md で一致しているか確認:
    ...
    - **ready sentinel は wake 能力の証明にならない**旨が書かれていること（watcher プロセスの生存しか示さず、idle セッションへ注入できる仕組みの下でも素のバックグラウンドシェルの下でも同じ sentinel が書かれる。検証結果は `docs/superpowers/specs/2026-08-12-delivery-verification-results.md`）。呼び出し元は prewarm.json の `delivery` 値で分岐しない（sentinel の確認は `send-prompt.sh` 自身が行う）。sentinel パスの agent 名側は既に `^[A-Za-z0-9._-]+$` 検証済みでエンコード不要だが、team 名は slug 制約が無いため `agmsg-path.sh`（guard と `send-prompt.sh` で共有）が team 側だけ percent-encode すること
    - `prewarm-panes.sh` が配線失敗（`DESIGN_DELIVERY=cmux-send` 等）時に **4 ロールとも guard 呼び出しを注入しない**こと。代替の文面はロール別で、**design / claude executor** は「直接タイプされる」フォールバック文面へ切り替わり、**codex executor / review** は初期プロンプトを渡さず idle 起動する（`${CODEX_EXEC_PROMPT:+...}` / `${REVIEW_PROMPT:+...}` が空で引数ごと落ちる。この 2 ロールは guard 導入前からプロンプト無し起動）。`/agmsg actas` は現行プロトコルに存在しない。配線成功時のプロンプトは 4 ロールとも「タスクはタイプ入力で届く（inbox に同一コピーあり）」文面であり「agmsg message として届く」とは書かないこと
    - 回帰は `bash test/test-send-prompt-callsites.sh`（CS1-CS3）、`bash test/test-send-prompt.sh`（SP0a-SP0d / SP1-SP24 / SP25-SP26）、`bash test/test-launch-workspace-codex.sh`（LW1-LW2）、`bash test/test-prewarm-layout.sh`（PW1-PW10）で検証する
...
20. codex の engine × MODE 起動規則を確認: superpowers は bypass 付き、review は `--sandbox workspace-write` + `-c approval_policy='never'` に加えて `--add-dir <STATUS_DIR>` / `--add-dir <AGMSG_SKILL_DIR>/run` / `--add-dir <AGMSG_SKILL_DIR>/db` の3本の `--add-dir` を条件付きで併用し、sandbox 完全 off を使わず findings 書込先と agmsg run/db ディレクトリへのアクセスを許可すること。`run/` と `db/` は `-d` 判定の**前に** `launch-workspace.sh` 側（サンドボックス外）で `mkdir -p` しておくこと — 実 `watch.sh` は `run/` を初回起動時に自分で作る設計だが、workspace-write のサンドボックス内からは作れないので、agmsg を新規インストールしたばかりの環境では `--add-dir` が付かず guard が恒久的に `reason=pidfile-missing` に落ちる。ただし `$AGMSG_SKILL_DIR` 自体が無い（agmsg 未インストール）ときはツリーを作らないこと。回帰は `bash test/test-launch-workspace-codex.sh` の CR1/CR1b/CR1c/CR1d で検証する

    **トラストバウンダリ（`--add-dir <AGMSG_SKILL_DIR>/{run,db}`）**: この付与は「このディスパッチ限り」ではなく**マシン全体の agmsg 状態への書き込み許可**である。`run/` には全セッション・全プロジェクトの pidfile / ready sentinel / actas lock が同居し、`db/` は全 team 共有の 1 つの SQLite DB **に加えて** agmsg のマシン全体設定 `config.yaml` と、外部ストレージドライバの opt-in allowlist `trusted-plugins`（`storage.sh` が `.` でソースするファイルの信頼リスト）も置かれる。したがって無人・承認なしの codex reviewer は、他ディスパッチの sentinel を消す / 任意の `from` を騙るメッセージを inbox へ書く（inbox の本文は他エージェントへテキストとして注入されるので**プロジェクト境界を越えるプロンプトインジェクション経路**になる）ことが原理上できる。guard を注入する以上この付与は機能上必須であり、`scripts/`（そこは全ペインの guard が実行するコード）を除外していることが**主要な**緩和策である。ただしそれが唯一ではない: `trusted-plugins` 経由で外部ドライバを登録できても、ドライバの探索元は環境変数 `AGMSG_PLUGIN_DIRS` **のみ**（`config.yaml` からは設定できず、既定では未設定）なので、他セッションの環境でこの変数が攻撃者の書けるディレクトリを指していない限りその経路は不活性である。実運用の到達性は低い**残存リスク**であって、開いた攻撃経路ではない。範囲を狭める案は agmsg 側が per-session のディレクトリを持たない限り成立しない
...
25. **permission prompt 抑止の settings 注入**が `launch-workspace.sh` / `SKILL.md` / `guide-ja.md` / `README.md` で一致しているか確認:
    ...
    - codex engine には**一切注入しない**こと。codex の `--dangerously-bypass-approvals-and-sandbox` とレビューペインの `--sandbox workspace-write` + `-c approval_policy='never'` + 3本の `--add-dir`（`<STATUS_DIR>` / `<AGMSG_SKILL_DIR>/run` / `<AGMSG_SKILL_DIR>/db`）は不変（項目 20 / 39）
...
17. **`AGMSG_INSTALLED=true` のとき**: monitor-dispatch.sh が起動しないこと。子が status.json に done/error を書いた**直後**に `[dispatch] task ... finished` が `send-prompt.sh` のタイプ入力で親に届き(親が idle のままでも届くこと — これがタイプ入力を唯一の wake 手段にする理由)、親の watcher が生きていれば同一文が inbox にも記録されること。wrapper の exit 時通知で同じ通知が重複して届くことがあるのは正常。ディスパッチ実行中の親セッションで、Step 1g が `ensure-agmsg-ready.sh` を直接呼び出す guard により watcher が起動していること（`AGMSG-DIRECTIVE:` に人手で従う経路も `Monitor` tool も不要）。guard の配線が失敗した場合は `AGMSG_INSTALLED=false` へ降格し `monitor-dispatch.sh` が起動すること。status.json は従来どおり遷移すること
18. **pre-warm**: `prewarm.json` の role-aware な `design` / `review` / `executors`（`executors` のキーは `claude` / `codex` の最大 2 つ）と実ペインが一致すること。各ロールのオブジェクトの `watcher` キーが、guard 呼び出しを注入したペインで `"guard-injected"`、配線失敗、または `SCRIPT_DIR` の空白・シェルメタ文字で注入できなかったペインで `"none"` になっていること。ペインが 2 行グリッド（上段 design / review、下段に実装ペインを横並び）で配置され、実装 2 件 + review のときに 2×2 に見えること。固定 `exec_choice` では未選択 executor を作らず、all-Codex 構成では設計・レビュー・codex executor の3ペインだけを起動すること。in-session 条件（`EXEC_ENGINE == DESIGN_ENGINE && EXEC_MODEL == PLAN_MODEL && EXEC_EFFORT == PLAN_EFFORT`、既定値を埋めた後で判定）が成立するタスクは `executors` が `{}` になり実装ペインが 1 つも起動しないこと。agmsg インストール時は design を含む起動済みペインが idle で待ち、親からの `send-prompt.sh --label phase-a-task`（`.assigned-<slug>` touch 後）で Phase A が始まること。`prewarm: false` では起動しないこと。未割当ペインが status.json を汚さないこと。claude design standby の起動が `--runner "$DESIGN_RUNNER"` を渡し、claude runner の `plan_model` / `plan_effort` が design ペインへ届くこと
...
27. **watcher 死亡時のフォールバック**: `delivery: "agmsg"` のタスクで宛先ペインの watcher を kill（ready sentinel が消える）した後に指示を送ると、inbox 記録がスキップされタイプ入力のみで配送されること（watcher 生存時もタイプ入力は常に行われる — inbox 記録の有無だけが変わる）。`send.sh` 自体が失敗した場合も配送は継続し終了コードは 0 のままであること。配線失敗（`delivery: "cmux-send"`）タスクは design / claude executor / codex executor / review の**全ロール**で初期プロンプトに guard 呼び出しが含まれないこと。うち **design / claude executor** は「typed directly into this pane」の文面になり、**codex executor / review** は初期プロンプトを渡さず idle 起動すること（guard 導入前からの挙動）。`/agmsg actas` は現行プロトコルに存在しない。配線成功タスクの初期プロンプトは「task will arrive as a prompt typed into this pane（inbox に同一コピー）」の文面であること
...
39. **codex 起動安全性**: superpowers は bypass で approval prompt を出さず、review は `--sandbox workspace-write` + `-c approval_policy='never'` に加え `--add-dir <STATUS_DIR>` / `--add-dir <AGMSG_SKILL_DIR>/run` / `--add-dir <AGMSG_SKILL_DIR>/db` の3本の `--add-dir` を条件付きで併用し、worktree 外の `<STATUS_DIR>/review/` への findings 書込みと agmsg run/db ディレクトリへのアクセスだけを許可すること（`AGMSG_SKILL_DIR/scripts` は書き込み許可に含めないこと）。`bash test/test-launch-workspace-codex.sh` の静的検査（CR1/CR1b を含む）を実行すること
