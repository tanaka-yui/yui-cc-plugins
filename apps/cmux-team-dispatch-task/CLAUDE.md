...
9. 配送が `scripts/send-prompt.sh` に一本化され、`cmux send` / `cmux send-key` の直書きが残っていないことを確認（`launch-workspace.sh` / `monitor-dispatch.sh` の実装、および SKILL.md / guide-ja.md の指示文。`send-prompt.sh` 自身が唯一の例外）。回帰は `bash test/test-send-prompt-callsites.sh` で検証する: CS1 = 両スクリプト、CS2 = 両スクリプトが `send-prompt.sh` を呼ぶ、**CS3 = SKILL.md と guide-ja.md の両方**に `send-key` が残らない（訳が原文から遅れて 2 行ペアを残す事故を防ぐため、対象を訳側にも広げてある）
