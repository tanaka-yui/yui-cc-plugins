...
11. Phase B は `phase-b-deliver.sh` が検証済み `prewarm.json.exec` の固定 tuple から agent / engine を使い、同 agent へ `phase-b-exec:` を 1 通送るだけで、新しい execute session を起動しないことを SKILL.md / guide-ja.md / README.md / CLAUDE.md で一致させる。**引き継ぎ文の自作を禁止する**ことも 4 ファイルで一致させる: Phase B-R の配線（レビュアーの agent 名 / review dir / round ファイル規約）は helper が組み立てる `phase-b-exec:` の本文にしか存在せず、設計ペインが自作すると情報ごと消える。2026-08-31 実測（`lead-psp-liff` の `member`）: 手書きの「PHASE B」にはレビュアー名も `review-code:` 手順も無く、代わりに「進捗と blocker は parent へ報告せよ」「`.dispatch/**` はこの worktree に存在しないので参照するな」と書かれていた。実装者は `parent` へレビューを依頼し、`exec_review` の inbox は参加以来 0 通、`code-round-*` は 1 つも生まれなかった（同タスクの Phase A-R は 55 ラウンド正常）。`.assigned-<exec agent>` / `.deferred` / `executing` status も揃って欠けた（書くのは helper だけ）。多重防御を 3 層で持つこと: (1) `launch-workspace.sh` の `write_dispatch_handoff()` が各 worktree へ `.dispatch-handoff.json`（status_dir / review_dir / review_config / team / agent / role / send_command）を書き、`info/exclude` にも入れる（worktree に `.dispatch/` は無いので、本体チェックアウトを指すポインタが要る）。ベストエフォートで失敗しても dispatch を止めない。(2) `completion-gate.sh` の判定 7 は、role が `exec` かつ `review/code-review.json` が読めて `code-round-*-request.md` が 1 つも無いとき、reason にレビュアーの agent 名・findings パス・「parent や surface id へ送るな」・request ファイルを先に書く手順を足す。(3) 親は起床のたびに「`code-review.json` があるのに `.assigned-<exec agent>` が無い」タスクを配線の失敗として検出し、停滞と区別して報告する。回帰は `test-completion-gate.sh` の CG36-CG39 と `test-launch-workspace-review-config.sh` の HO1-HO4 で固定する。送信前の assignment marker、送信成功後の `.deferred`、実装者用 parallel directive、terminal status と `dispatch-notify:`、engine 別の終了規則を依頼文に含める。`launch-workspace.sh --mode execute` は launcher 単体の回帰用経路として残るが dispatch の Phase B からは呼ばない
...
14. Phase A-R（設計セッションの plan/spec を `design_review` がレビュー）が4ファイルで一致しているか確認:
    ...
    - **レビューペインに assignment marker を作らない**（`.assigned-<design_review agent>` を含む review ロールの `.assigned-*` は 1 つも touch しない）。review ペインは standby 起動で、runner wrapper は standby の所有権を `.assigned-<slug>` の有無だけで判定するため、作ると review ペインが共有 `status.json`（他ロールの結果）を自分の終端状態として親へ通知し、`abort-reviewer:` まで送る。2026-08-28 に実測: 1 ロールの失敗が 3 ロールの失敗として親へ届き、まだ作業中の `exec_review` へ abort が飛んだ。回帰は `test-review-contract-docs.sh` の RD7（文書側）と `test-runner-terminal-status.sh` の R6（runner 側）で二重に固定する
    - **レビュー依頼は `scripts/review-request.sh` への 1 コールで行う唯一の経路**。本文は stdin で渡し、helper が `<point>-round-<N>-request.md` の書き込みと接頭辞付き `review-plan:` の送信をまとめて行い（`send.sh` を直接呼ばない — 接頭辞は helper 側で付くため、呼び出し側で付けると二重になる）、送信に失敗したら書いたファイルを削除する。completion gate はディスクだけを読むので、このファイルだけが「verdict 待ち」の証拠になる（詳細は項目 33）
...
17. Phase B-R（実装後コードレビュー）が SKILL.md / guide-ja.md / README.md / CLAUDE.md の 4 ファイルで一致しているか確認:
    ...
    - 実装者は **送信前に依頼文を `<STATUS_DIR>/review/code-round-N-request.md` へ書いてから** `exec_review` agent へ `review-code:` を送り、reviewer は `<STATUS_DIR>/review/code-round-N.md` の末尾に `VERDICT: approve` または `VERDICT: needs_work` を書いてから `review-verdict:` を返す。最大 5 ラウンドで、第 6 ラウンドは開始しない。round 5 が needs_work なら未解決指摘を PR 本文へ記録して進む。request ファイルを書かないと verdict 待ちの実装者が完走ゲートの判定 7 に落ちる（項目 33）
    - **中断時の停止理由は `<point>-round-<N>-abort.md` へ書き、reviewer の findings パスへは書かない**。findings は reviewer の出力先で、2 人の書き手のあいだに順序保証が無い（2026-08-28 実測: 中断メモが進行中のレビュー結果を潰し、親が `-final.md` へ退避させた）。reviewer のゲート解放は gate 側の `round_aborted()` が引き受ける
...
23. **error パスの通知**が SKILL.md / guide-ja.md / README.md / CLAUDE.md の 4 ファイルで一致しているか確認:
    ...
    - 実装者の ABORT/ESCALATION プロトコル（**abort 記録** → レビュアー通知 → status.json → 親通知 → セッション終了）が prewarm 経路と spawn 経路の両方に入り、unattended 文面にも同じ手順があること。**記録先は `<point>-round-<N>-abort.md`** で、reviewer の findings パス（`code-round-N.md`）ではないこと（項目 17）
    - **`--mode review` の runner は status.json の書き込みも通知も一切行わないこと**。watcher を起動せず、exit パスは standby 判定より前に無条件で抜ける。marker が何であれ review ペインはタスクを所有できず、それは MODE から静的に分かる（項目 14 の実測事故に対する多重防御）
...
33. **完走ゲート**: 4 ロールとも Stop hook (`completion-gate.sh`) が注入され、判定がディスクだけを読むこと。**「待って良い状態」を block しない**こと — タスク未着（`design`/`exec` は `.assigned-<agent>` 無し、review ロールは `review/*round*.md` 無し）と verdict 待ち（依頼側で `VERDICT:` 行が無い、または最新の `<point>-round-<N>-request.md` が findings より新しい）は停止を許す。**依頼が request ファイルとして materialize されない限り待機は見えない** — Phase B-R は依頼を agmsg メッセージだけで送っていたため、round 2 以降の verdict を待つ exec が判定 7 に落ち、110 秒で block 文面が勧める `error` を書いて中断した（2026-08-28 実測）。したがって Phase A-R / B-R とも `scripts/review-request.sh` への 1 コールで request ファイルの書き込みと送信をまとめて行い、**判定 7 の reason 自身が「verdict 待ちは block でも error でもない」と述べて request ファイルを名指しする**（書き忘れた待機者を自己修復する層）。`error` は作業そのものが失敗したときに限る。**打ち切りは `<point>-round-<N>-abort.md`** に記録し、request と findings の両方より新しい abort ファイルは「そのラウンドは終わった」を意味して、書きかけの findings を抱えたレビュアーの停止も許す（`round_aborted()`）。abort ファイルは findings としても request としても読まない。レビュアーは同じ「`VERDICT:` 行が無い」状態で逆に block する（自分がまだ書いていない意味だから）。**待機を allow するだけでは足りない**こと — codex の goal 継続（feature flag `goals`）はターン終了の数十ミリ秒後に `<codex_internal_context source="goal">` を注入して次のターンを始め、その注入文自体が「同じ blocking condition が**自動継続を含めて**3 連続 goal ターンで続いたら `update_goal(status:"blocked")` を宣言せよ」という codex 自身の blocked audit を含む。レビュー待ちは 30 秒足らずでこれを満たす（2026-08-31 実測: `review-code:` の 81 秒後に継続 6 回で abort、別 dispatch で 111 秒後に継続 13 回で abort。どちらも直後に `update_goal({status:"blocked"})` を実行し、レビュアーは 2 件とも正常に完走した。同一セッションで goal が非アクティブな区間では同じ待機が 90 分継続している）。したがって**自動再開されている待機だけ block へ倒し**、`reason` で経過分・abort 禁止・terminal status 禁止・`update_goal(status:"blocked")` 禁止・「継続はタイマーでも wake でもない」を述べること。**素の待機を block しない**こと — 判別はスタンプ `<status-dir>/.gate-wait-<role>` の間隔で行い、`DISPATCH_GATE_WAIT_RESTART_SECONDS`（既定 90 秒）以内の再訪だけを自動再開と見なす（再開する者が居なければ 2 回目の Stop はそもそも来ない）。防衛は依頼ファイルの mtime から `DISPATCH_GATE_WAIT_MINUTES`（既定 30 分）で失効し、既存の打ち切り経路を残す。`0` で無効化。スタンプは待機経路を通らなかった停止でだけ消すこと（`allow` / `block` の両方で消すが、`wait_guard` を通った実行では消さない — 消すと自分が今書いた時刻が毎回消え、次の呼び出しが永久に「1 回目」になる）。**ここに新しいポーリングループを足さないこと** — 数えているのは engine が勝手に起こしたターンである。**駆動源そのものは起動時に切ること** — `launch-workspace.sh` は全 codex 経路（superpowers / plan / execute / standby / review の 6 箇所の `CORE_CMD`）に `CODEX_GOALS_FLAG=" -c features.goals=false"` を付ける。これが確定的な側で、ゲートの `reason` は goal が別経路（動作中ペインの TUI で人が設定するなど）で復活したときの第 2 層。代償は止まった codex ペインが自力再開しなくなることで、復帰は agmsg 経路（`review-verdict:` / 停滞検知の `dispatch-nudge:`）に一本化される（設計上もともとその前提）。claude 経路には**付けないこと**。回帰は `test-launch-workspace-codex.sh` の T16（codex 5 モードに付く）/ T16b（claude 5 モードに付かない）で検証する。連続 block に**既定の上限は無い**こと — 有限の上限は「まだ終わっていない長いタスク」を殺す（上限に達した block はカウンタを保持したまま諦め、カウンタを消せるのは判定 1-5 の allow だけなので、実装中の exec はどの allow にも二度と当たらず以後永久に毎ターン停止する。2026-08-25 に実ペインで観測）。`DISPATCH_GATE_MAX_BLOCKS` に正の数を入れたときだけ上限が働き、カウンタは `<status-dir>/.gate-blocks-<role>` と**ロールごとに分ける**こと（4 ロールが 1 つの status dir を共有するので、1 ファイルだと「4 ペイン合計で上限」になる）。上限有効時は**上限到達でカウンタを消さない**こと（消すと即座に再武装され「上限まで block → 1 回休み」を繰り返す）。**ロールを hook の command に焼き込まない**こと — 4 ロールは 1 worktree を共有し engine ごとの hook ファイルは 1 本なので、焼き込むと 2 ロール目が 1 ロール目のゲートを実行する（2026-08-25 実測: `exec_review` が `design` のゲートで動いた）。値は runner script が `DISPATCH_GATE_ROLE` / `DISPATCH_GATE_AGENT` / `DISPATCH_GATE_STATUS_DIR` / `DISPATCH_GATE_TEAM` として export し、gate が引数の無いときに読むこと（command 内の `'$VAR'` は hook 実行時にも展開されないので代替にならない）。**identity が揃わなければ fail-open** で `exit 0` かつ stdout 無出力にすること（Claude Code の Stop hook では `exit 2` は blocking error であって no-op ではない）。注入は**既存のゲート entry を除去してから 1 本足す**こと — これで worktree 再利用の二重注入と 3.6.0 以前の焼き込み entry の migration が同時に片づく。ベストエフォートで、`.codex/hooks.json` は agmsg の entry とマージし `info/exclude` に入ること
    **停止済みペインの期限は gate ではなく runner の既存 watcher から呼ぶ `recovery-tick.sh` が執行する。** gate の単一 writer は削除しない `.gate-seq-<role>` と `.gate-wait-<role>`、tick の単一 writer/deleter は `.gate-nudge-<role>` であり、削除も書き込みである。tick の順序は hard closure → `.escalated` → soft closure → lease 欠落 → generation 変化 → state（`.escalated` は verdict 後に上限到達し得るため soft closure より先）。`DISPATCH_GATE_WAIT_MINUTES=0` は gate の wait 防衛と tick の両方を無効化する。`.escalated` を作る子は touch せず `scripts/escalate.sh <STATUS_DIR>` を 1 回呼び、原子的な token で再エスカレーションを識別する（素の touch は互換入力だが再作成を識別できない）。通知の送信成功後に status dir への記録が失敗した場合、書けるまで tick ごとに再送するので、I18 / T49 の重複排除保証は writable な status dir を前提とする。回帰は CG2 更新、CG40–CG59、RS1–RS18、RT1–RT27、CI13–CI22 で固定する。
...
39. **codex 起動安全性**: superpowers は bypass で approval prompt を出さず、review は `--sandbox workspace-write` + `-c approval_policy='never'` に加え writable_roots へ `<STATUS_DIR>/review` / `<AGMSG_SKILL_DIR>/run` / `<AGMSG_SKILL_DIR>/db` の 3 つだけを条件付きで載せること
