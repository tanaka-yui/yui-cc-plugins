...
| `skills/cmux-team-dispatch-task/scripts/agmsg-path.sh` | agmsg の ready sentinel パス（`ready.<team>__<agent>`）を組み立てる source 専用ヘルパー。**`send-prompt.sh` 専用**（team 名の percent-encoding が要るのは team 側だけ）。guard は `--name` を `^[A-Za-z0-9._-]+$` に値域検証済みでエンコードが恒等写像になるため、依存を持たない |
