...
**block に既定の上限は無い。** `DISPATCH_GATE_MAX_BLOCKS` に正の数を入れたときだけ上限が
働き、連続 block を `<status-dir>/.gate-blocks-<role>` に数える。既定を無制限にしたのは、
有限の上限が「まだ終わっていない長いタスク」を殺すからである。上限に達した block は
カウンタを保持したまま諦め、カウンタを消せるのは判定 1-5 の allow だけなので、実装中の exec
（status=executing かつ最新ラウンドに VERDICT が既にある = 待機でもない）はどの allow にも
二度と当たらず、以後永久に毎ターン停止する。2026-08-25 に実ペインで観測した。

上限を有効にしたときは **諦めるときにカウンタを消してはならない** — 消すと上限が即座に
再武装され、「上限まで block → 1 回休み」を延々と繰り返す。リセットするのは本当に停止を
許したときだけで、タスクが待機か終端に入れば自然に回復する。カウンタをロールごとに分けるのは、
4 ロールが 1 つの status dir を共有するからである。1 ファイルに数えると「4 ペイン合計で上限」
になり、どのペインも自分の回数に達する前に諦める。

**ゲートが知っている値はすべて `reason` に入れる。** reason は次ターンのガイダンスとして届き
モデルがそれに従うので、条件だけ書いて満たすための値を渡さないとセッションは探索を始める。
実ペインで計測した: 条件だけの reason では、block されたセッションが `ls ..` を打ち、
`completion-gate.sh` と `report-status.sh` を読み、見つからない team 名を探す考古学にまる 1
ターンを費やし、通知は諦めた。status dir・`report-status.sh`・agent・team・`send.sh` の
...
（選択結果は round N の findings で、すでに `VERDICT:` を持つ）である。どちらも「verdict 待ち」
の条件に当たらないため最終判定へ落ち、書いてはいけない terminal status を書けと迫られる。
2026-08-26 に実ペインで観測した。そこでゲートは最新の `<point>-round-<N>-request.md` も見て、
