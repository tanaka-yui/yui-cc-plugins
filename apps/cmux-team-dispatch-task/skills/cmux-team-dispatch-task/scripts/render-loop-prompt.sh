...
printf '\n%s\nPhase B: delegate implementation to the configured execution role above. Mark that role as the status owner before delivery, and do not infer its runner or engine from another role. Deliver the complete execution request with ONE call to `bash <skill directory>/scripts/send-prompt.sh --to-surface <target pane surface> --label phase-b-exec --outbox-dir <status directory>/outbox -- <request text>`; never type it into a pane yourself.\n\nIssue body:\n' "$phase"
cat "$BODY_FILE"
