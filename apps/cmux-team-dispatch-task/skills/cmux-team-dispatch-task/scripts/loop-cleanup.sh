...
apply_labels() {
  local issue="$1" status="$2" terminal
...
for issue in $(jq -r --argjson batch "$BATCH" '.issues | to_entries[] | select(.value.batch == $batch) | .key' "$STATE_FILE"); do
  ...
  if [[ "$status" == done && "$INTEGRATION" == merge ]]; then
    if git -C "$REPO_ROOT" merge "feat/$slug" --no-edit >/dev/null 2>&1; then merged_count=$((merged_count+1)); else git -C "$REPO_ROOT" merge --abort >/dev/null 2>&1 || true; status=error; conflicted_count=$((conflicted_count+1)); fi
  fi
  ...
  if ! apply_labels "$issue" "$status"; then record_leak "$slug: terminal ラベル付与に失敗"; continue; fi
