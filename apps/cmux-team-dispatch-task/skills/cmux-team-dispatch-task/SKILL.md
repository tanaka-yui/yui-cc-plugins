...
another. A fixed all-Codex unattended task uses codex for all four roles and creates no
...
Before any pane is launched, require ~/.agents/skills/agmsg/scripts/send.sh and determine
the parent engine from the current runtime. A Claude parent verifies the Monitor watcher;
...
# A codex parent's reachability is keyed by team and agent name, so resolve both first.
...
A Codex parent has no persistent Bash timer: background subshell and nohup timers die with
the turn. Never claim to arm one. Refuse unattended dispatch from a Codex parent because
an all-Codex unattended run has no safety backstop. A Claude parent may arm one
single-shot 90-minute sleep, never a polling loop.
...
| review-timer: / dispatch-timer: | reserved; nothing emits these today |

Join the parent before launch:
