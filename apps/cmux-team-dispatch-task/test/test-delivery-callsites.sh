...
# monitor-dispatch.sh / batch-wait.sh は後続タスクが削除するまで存在するので、
# その 2 つを CS4 の対象へ加えるのは削除タスク側の仕事である。
#
...
PENDING=(
  ...
  "skills/cmux-team-dispatch-task/scripts/monitor-dispatch.sh|4|T4 (monitor-dispatch の配送を Monitor push へ移す)"
