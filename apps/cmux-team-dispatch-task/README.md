...
届いていないペインと、依頼済みの verdict を待っているペインは、そのまま待たせます。
...
**engine が自分でターンを再開する場合、待機を許すだけでは足りません。** codex には goal 継続機能
（feature flag `goals`）があり、ターン終了の数十ミリ秒後に次のターンを注入するため、待機中の
ペインが 7〜10 秒周期で回り続けます。しかもその注入文自体に codex 側の「同じ blocking condition が
**自動継続を含めて** 3 連続で続いたら blocked を宣言せよ」というルールが入っているので、レビュー
待ちは 30 秒足らずでその閾値に達します（2026-08-31 実測: レビュー依頼の 81 秒後と 111 秒後に
`exec` が中断。レビュアーはどちらも正常で、あとから完走しました。同じセッションで goal が
動いていない区間では、同じ待機が 90 分そのまま続いています）。ゲートはこの場合だけ `block` へ倒し、
「経過は何分で、これは待機であって障害ではない。abort も terminal status も blocked 宣言も書くな。
自動再開はタイマーでも wake でもない」と伝えます。素の待機は縛りません（再開する者が居なければ
2 回目の停止判定がそもそも来ないので、`<status-dir>/.gate-wait-<role>` の間隔で見分けられます）。
猶予は依頼ファイルの mtime から 30 分（`DISPATCH_GATE_WAIT_MINUTES`。`0` で無効化、
再開と見なす間隔は `DISPATCH_GATE_WAIT_RESTART_SECONDS`、既定 90 秒）で、使い切ると従来どおり
打ち切り経路へ戻ります。ポーリングは増えていません — 数えているのは engine が勝手に起こした
ターンです。

そのうえで**駆動源そのものも起動時に切ります**。codex ペインは全経路で
`-c features.goals=false` を付けて起動するので、goal 継続は最初から動きません。ゲート側の
防衛は、goal が別経路（動いているペインの TUI で人が設定するなど）で復活したときのための
第 2 層として残しています。代償は、止まった codex ペインが自力で再開しなくなることです。
復帰は元からの agmsg 経路（`review-verdict:` メッセージ、または停滞検知による親の
`dispatch-nudge:`）に一本化されます。元の挙動へ戻したいときは `launch-workspace.sh` の
`CODEX_GOALS_FLAG` を空にしてください。
...
**claude の子セッションだけ**が、独立した調査と検証を Task サブエージェントへ分散させるよう
指示される。ファイル編集は親エージェントで逐次のままなので、同一 worktree での書き込み競合は
起きない。分散してよいのは**読み取り専用の検証だけ**で、auto-fix / write モード（formatter や
linter を write フラグ付きで走らせるもの）は親エージェントで逐次に実行させる。

**codex の子セッションには並列を指示しない。** codex の子エージェントは shared local
app-server daemon 上の別スレッドとして走り、ペインには一切映らない（覗けるのは `codex agents`
という別 TUI）。ペインを見ている限り「4 体が動いている」のか「1 体も動いていない」のか
区別できず、これが「codex が動いているのか止まっているのか分からない」の主因だった。
なお、これは**指示しない**という限定であって、codex が**できない**という保証ではない。
collaboration tools は `features.multi_agent_v2 = false` でも登録されたままで、設定キーでは
消せない（codex-cli 0.149.1 で実測）。黙り込んだ codex 子は後述の停滞検知が捕まえる。

同時に走る子エージェントの上限は既定 4。これを変える `--agents <N>`（2〜8）と、
起動プロンプトへの指示を止める `--no-parallel` は、いずれも
**`launch-workspace.sh` のスクリプトレベルのフラグ**で、**効くのは claude だけ**である
（codex は生成器が常に空を返すので自然に no-op になる）。スキルはこの 2 つを
...
対話 codex のペインは終了しないので、「ペインが生きている」は進捗の証拠にならない。親が
タイマーで起きたとき、`scripts/work-signal.sh` が HEAD のコミット / dirty なパスの集合 /
それらの mtime / ペインの画面をハッシュ化し、前回の起床時と比較する。

| 観測 | 判定 | 動作 |
|------|------|------|
| `changed=yes` | 動いていて黙っているだけ | 再武装（**再武装カウンタを増やさない**） |
| `changed=no` かつ受信可能 | 停滞 | `dispatch-nudge:` を 1 通だけ送って自動再開 |
| `changed=no` かつ seat 未記録 | 止まっていて、かつ届かない | nudge せず切り分け結果を報告 |
| `changed=unknown` | 画面が読めず判断不能 | ペイン死亡の確認へ回す |

**新しいポーリングループは 1 本も増えない。** 検査の契機は元からある単発タイマーの起床だけで、
nudge は 1 タスクにつき 1 回まで。それでも動かなければ `read-screen` の抜粋を添えて報告する。
