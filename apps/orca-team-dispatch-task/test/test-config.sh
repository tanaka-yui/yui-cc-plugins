...
RESOLVE="$S/config-resolve.sh"; EDIT="$S/config-edit.sh"
...
setup
echo '{not json' > "$G"
bash "$RESOLVE" --project-root "$PR" >/dev/null 2>&1
...
setup
bash "$EDIT" --config "$G" --set roles.design.agent=codex --set roles.design.model=gpt-6-astra \
...
bash "$EDIT" --config "$G" --set roles.design.model=sonnet >/dev/null 2>&1
...
before=$(cat "$G")
bash "$EDIT" --config "$G" --set roles.design.model=opus --set roles.design.effort=minimal >/dev/null 2>&1
rc=$?
[[ "$rc" -eq 2 && "$(cat "$G")" == "$before" ]] \
  && ok "CF13 不正値のバッチは 1 つも書かない" || fail "CF13 (rc=$rc)"
bash "$EDIT" --config "$G" --set roles.bogus.agent=claude >/dev/null 2>&1
...
bash "$EDIT" --config "$G" --unset roles.design.effort --unset roles.design.model >/dev/null 2>&1
...
setup
[[ -z "$(bash "$EDIT" --config "$G" --get roles.design.model 2>/dev/null)" ]] || fail "CF15 未設定の --get"
[[ "$(bash "$EDIT" --config "$G" --show 2>/dev/null)" == '{}' ]] || fail "CF15 不在の --show"
bash "$EDIT" --config "$G" --set roles.design.model=sonnet >/dev/null 2>&1
[[ "$(bash "$EDIT" --config "$G" --get roles.design.model 2>/dev/null)" == sonnet ]] \
...
bash "$EDIT" --config "$G" --set roles.design.effort=minimal >/dev/null 2>&1
[[ $? -eq 0 ]] || fail "CF16 既存 config の agent を見なかった"
bash "$EDIT" --config "$G" --set roles.design.agent=claude --set roles.design.effort=minimal >/dev/null 2>&1
...
setup
[[ "$(bash "$RESOLVE" --project-root "$PR" 2>/dev/null | jq -r .configured)" == false ]] \
  || fail "CF17 設定ゼロ"
