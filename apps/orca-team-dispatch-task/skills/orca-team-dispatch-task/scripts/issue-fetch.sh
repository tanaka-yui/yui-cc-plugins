...
#   上流から変えたのは次の 4 点だけである。それ以外は byte 一致で持ち込んでいる
...
case "$SUBCOMMAND" in
  ...
  reconcile)
    ...
    for number in $(jq -r '.issues | to_entries[] | select(.value.status == "claimed") | .key' "$STATE_FILE"); do
      ...
      [[ -f "$DISPATCH_DIR/$slug/workers.json" ]] && evidence="${evidence:+$evidence, }workers.json"
      ...
      if [[ -f "$DISPATCH_DIR/$slug/workers.json" ]]; then
        while IFS= read -r wtp; do
          [[ -n "$wtp" && -d "$wtp" ]] || continue
          evidence="${evidence:+$evidence, }worktree"
          break
        done < <(jq -r '.roles[]?.worktree_path // empty' "$DISPATCH_DIR/$slug/workers.json" 2>/dev/null)
      fi
      if [[ -n "$evidence" ]]; then
        action=abort
    ...
    for label in dispatch/in-progress dispatch/done dispatch/failed; do
      if [[ "$(jq -r --arg label "$label" '[.[] | select(.name == $label)] | length' <<<"$existing")" == 0 ]]; then
        gh label create "$label" --description "orca-team-dispatch-task issue mode" >/dev/null 2>&1 || die "ラベル '$label' の作成に失敗しました"
      fi
    done
    ;;
  finalize)
