...
import { asArray, asString, get } from './json.ts'
...
//   Orca が返す端末 ID も記録する（最新の試行を片付ける際に必要）。
...
export const startIncomplete = (statusDir: string, role: string): boolean => {
  ...
  // 現在の試行自身の worker_done は、起動できたことの確証。前の試行の receipt は効かない
  const task = field('task')
  const receipt = `worker_done|${task}|${field('dispatch')}|`
  if (
    task !== '' &&
    (asArray(readJson(join(statusDir, 'received.json'))) ?? []).some(
      (entry) => typeof entry === 'string' && (entry === `${receipt}succeeded` || entry === `${receipt}failed`),
    )
  )
    return false
