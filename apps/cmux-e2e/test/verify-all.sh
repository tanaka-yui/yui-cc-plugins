...
[[ "$(jq -r .version apps/cmux-e2e/.claude-plugin/plugin.json)" == 1.0.1 ]] || fail 'Claude version'
[[ "$(jq -r .version apps/cmux-e2e/.codex-plugin/plugin.json)" == 1.0.1 ]] || fail 'Codex version'
[[ "$(jq -r '.plugins[]|select(.name=="cmux-e2e")|.version' .claude-plugin/marketplace.json)" == 1.0.1 ]] || fail 'marketplace version'
