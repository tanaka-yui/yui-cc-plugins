...
auth_save_locked() { local ref="$1" name="$2" url="${3:-}" sel="${4:-}" d s m; [[ -z "$url" && -z "$sel" || -n "$url" && -n "$sel" ]] || return 2; d=$(_cmux_e2e_auth_dir "$name") || return $?; cmux_e2e_mkdir_secure "$d" || return 1; s="$d/.state.$$"; m="$d/.meta.$$"; "$CMUX_BIN" browser --surface "$ref" state save "$s" >/dev/null || return 1; "$CMUX_E2E_JQ" -n --arg sha "$(_cmux_e2e_digest "$s")" --arg url "$url" --arg sel "$sel" '{state_sha256:$sha,check_url:($url|select(.!="") // null),check_selector:($sel|select(.!="") // null)}' > "$m" || return 1; cmux_e2e_install_file "$s" "$d/state.json" && cmux_e2e_install_file "$m" "$d/meta.json"; }
...
auth_delete_locked() { local d; d=$(_cmux_e2e_auth_dir "$1") || return $?; [[ -d "$d" ]] && rm -rf -- "$d"; }
