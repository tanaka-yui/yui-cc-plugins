...
cmux_e2e_validate_name "$name" || exit 2
check_url=''; check_selector=''
while [[ $# -gt 0 ]]; do
  case "$1" in
    --check-url) [[ "$action" == save && $# -ge 2 ]] || exit 2; check_url="$2"; shift 2 ;;
    --check-selector) [[ "$action" == save && $# -ge 2 ]] || exit 2; check_selector="$2"; shift 2 ;;
    *) exit 2 ;;
  esac
done
[[ -z "$check_url" && -z "$check_selector" || -n "$check_url" && -n "$check_selector" ]] || exit 2
...
case "$action" in save) auth_save_locked "$ref" "$name" "$check_url" "$check_selector" ;; load) auth_load_locked "$ref" "$name" ;; check) auth_check_locked "$ref" "$name" ;; esac
