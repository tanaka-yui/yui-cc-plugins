...
#   D10. プロンプトに並列実行の語彙が 1 つも入らない（否定的不変条件）
#   D11. 削除済みフラグ --no-parallel / --agents は非ゼロ終了し、ペインを分割しない
#        （*) はレビュー指示本文へ落ちるので、明示的に拒否しないと黙って混ざる）
#   D12. 通知配線時、send.sh の本文に agents= が入らず、prompt は 1 引数のまま
#
# D10-D12 が否定形なのは、codex に並列を指示しなくなったため。codex の子エージェントは
...
# --- D10: プロンプトに並列実行の語彙が 1 つも入らない ---
CMUX_BIN="$TMP/bin/cmux" "$BIN" >/dev/null 2>&1
reparse
d10=1
for word in spawn_agent wait_agent 並列 子エージェント agent_type; do
  if prompt | grep -q "$word"; then
    echo "  並列語彙が残っている: $word"
    d10=0
  fi
done
if [[ "$(argc)" == "9" && $d10 -eq 1 ]]; then
  echo "PASS D10: プロンプトに並列実行の語彙が入らない、argc=9"
else
  echo "FAIL D10: argc=$(argc) / prompt=[$(prompt)]"
  fail=1
fi

# --- D11: 削除済みフラグは非ゼロ終了し、ペインを分割しない ---
# この bin の *) は追加レビュー指示へ落ちるので、明示的に拒否しないと --no-parallel が
# レビュー指示の本文に混ざったまま codex が起動し、効いていないことに気づけない。
rm -f "$TMP/split.log"
bad=0
for flag in --no-parallel "--agents 4"; do
  # shellcheck disable=SC2086
  SPLIT_LOG="$TMP/split.log" CMUX_BIN="$TMP/bin/cmux" "$BIN" $flag >/dev/null 2>&1 && bad=1
done
msg=$(CMUX_BIN="$TMP/bin/cmux" "$BIN" --no-parallel 2>&1 >/dev/null)
if [[ $bad -eq 0 && ! -f "$TMP/split.log" ]] && grep -q '削除されました' <<<"$msg"; then
  echo "PASS D11: 削除済みフラグを明示的に拒否し、ペインを分割しない"
else
  echo "FAIL D11: bad=$bad / split.log=$( [[ -f "$TMP/split.log" ]] && echo exists || echo none ) / msg=[$msg]"
  fail=1
fi

# --- D12: 通知本文に agents= が入らず、prompt は 1 引数のまま ---
...
reparse
if [[ "$(argc)" == "9" ]] && prompt | grep -q 'レビュー完了' && ! prompt | grep -q 'agents='; then
  echo "PASS D12: 通知本文に agents= が入らず argc=9 のまま"
else
  echo "FAIL D12: argc=$(argc) / prompt=[$(prompt)]"
  fail=1
fi
