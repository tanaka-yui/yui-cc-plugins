...
**待機の契約そのものは `/codex-review` コマンド（`commands/codex-review.md`）の Step 3 に従う**。
0 節・1 節が分岐をコマンドへ委譲しているのと同じ扱いである。Monitor 稼働の preflight、
競合 watcher の確認、タイマーの長さ、完了時の `TaskStop`、`token` の照合、タイマーで起きた
ときに何かを判断する前に必ず行う `history.sh` の読み取り、再武装の上限は、すべてコマンド側にしか
書かれていない。このファイルから推測で補ってはならない。
